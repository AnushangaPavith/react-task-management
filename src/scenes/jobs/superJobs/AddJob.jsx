  ...
  const handleFormSubmit = async (values, { resetForm }) => {
    ...
    try {
      ...
      const response = await axios.post(
        environment.apiUrl + `/super/addSuperTask`,
        values,
        { headers }
      );
            ...
            >
              ...
              <Box mt={-2}>
                <TextField
                  ...
                  name="taskStartTime"
                  error={!!touched.taskStartTime && !!errors.taskStartTime}
                  helperText={touched.taskStartTime && errors.taskStartTime}
              ...
              <Box mt={-2}>
                ...
                >
                  <Select
                    ...
                    sx={{ width: "50%" }}
                  >
                    {["daily", "weekly", "fortnight", "other"].map((option) => (
          ...
          >
            <Select
              ...
              onChange={(event) =>
                setFieldValue(
                  "days",
                  // Handle multiple selections by checking the event's target value
                  event.target.value
                )
              }
              value={values.days}
              name="days"
              error={!!touched.days && !!errors.days}
              disabled={values.type === "daily" || values.type === "weekly"} // Disable when type is daily or weekly
              renderValue={(selected) => (selected && selected.length > 0 ? selected.join(", ") : "Select days")}
              sx={{ width: "50%" }}
            >
              {[
                "Monday",
...
const jobSchema = yup.object().shape({
  ...
  taskStartTime: yup.string().required("Start Time is required"),
  ...
  days: yup.array().of(yup.string()).when("type", {
